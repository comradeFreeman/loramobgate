...
| [privateKey](https://github.com/comradeFreeman/loramobgate/blob/ce9ea6843d09e33fa841d488b3c35dac4adfd7fa/device/src/eeprom_vars.h#L9) | Закрытый ключ Устройства, применяется для шифрования в мессенджере (более подробно - дальше). |
...
> **Внимание!** Для того, чтобы работал поиск пакетов в `redis-om`, требуется не обычный сервер `Redis`, [`Redis-Stack-Server`](https://github.com/redis-stack/redis-stack)!
...
Для использования ранее разработанного ПО хоста, реализации возможности взаимодействия устройств между собой, коммуникации их владельцев и работы LoRaWAN-шлюза была выбрана концепция применения-мессенджера, которое будет предоставлять соответствующий пользовательский интерфейс, похожий на обычный мессенджер вроде Telegram, Viber, WhatsApp. Технически [приложение](https://github.com/comradeFreeman/loramobgate/tree/master/messenger) использует фреймворк [Kivy](https://github.com/kivy/kivy) и коллекцию виджетов в стиле Material Design v2 [KivyMD](https://github.com/kivymd/KivyMD), а для сборки в APK (Android) и IPA (iOS, **пока не реализован**) файлы используется проект [Buildozer](https://github.com/kivy/buildozer). В дальнейшем проект будет дополнен сценариями сборки под ОС Windows, Linux и macOS. Разработанное приложение имеет следующие экраны:
...
![disp_3](./docs/disp_3.png)

- Настройка применения. Этот экран содержит настройки графического интерфейса и работы ПО хоста.
