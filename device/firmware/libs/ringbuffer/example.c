...
void example_8bit(void){
    ...
    static uint8_t ringbuf8_data[20] = {0,}; // static data buffer
    RingBuf_Init(ringbuf8_data, 20, sizeof(uint8_t), &ringbuf8);
    uint16_t avail8 = 0;

    // Input array
    uint8_t in8[5] = {10, 15, 24, 255, 8};
    ...
    printf("Avail_ %u", (unsigned int)avail8);
    ...
    uint8_t out8_read = 0; // output value after reading
    // Read 1 byte
    ...
    uint8_t out8_watch = 0; // output value after watching
    ...
    uint8_t out8[5] = {0,}; // output array for reading
...
void example_16bit(void){
    ...
    static uint16_t ringbuf16_data[10] = {0,}; // static data buffer
    RingBuf_Init(ringbuf16_data, 10, sizeof(uint16_t), &ringbuf16);
    uint16_t avail16 = 0;

    // Input array
    uint16_t in16[5] = {3443, 1004, 1337, 2281, 1234};

    // Put 1 cell (16 bit variable)
    RingBuf_CellPut((uint16_t*)&in16[0], &ringbuf16); // {3443, ...}

    // Check availability after putting 1 cell
    RingBuf_Available(&avail16, &ringbuf16); // avail: 1

    // Put data from specific part of array
    RingBuf_DataPut((uint16_t*)&in16[2], 3, &ringbuf16); // {3443, 1337, 2281, 1234, ...}

    // Check availability after putting 3 cells
    RingBuf_Available(&avail16, &ringbuf16); // avail: 4

    uint16_t out16_read = 0; // output value after reading
    ...
    uint16_t out16_watch = 0; // output value after watching
    // Watch without flushing
    ...
    uint16_t out16[5] = {0,}; // output array for reading
...
void example_ovf(void){
    ...
    uint32_t data_in[3] = {727270, 917020, 812734};
    RingBuf_DataPut(data_in, 3, &ringbuf); // data: {727270, 917020, 812734}
    uint32_t data_out[3] = {0,};
