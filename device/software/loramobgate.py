...
# 		self.target_addr = kwargs.get('target_addr', settings.BROADCAST) # broadcast
...
# 			self.src_addr, self.target_addr, _, self.app_id, self.pydc, self.content_type = \
# 												unpack("<2I4B", bytes(source_packet[:12]))
# 			self.fragm_c, self.fragment, self.app_id = _ >> 4, _ & 0xf, AppID(self.app_id)
# 			self.encrypted_data = bytes(source_packet[12:12 + self.pydc])
# 			self._decrypt()
# 			self.update()
#
...
#
# 		# как только меняется незашифрованный текст, мы сразу шифруем его
# 		if key == "raw_data": # or (key == "app_id" and hasattr(self, "raw_data")):
# 			self._encrypt()
#
# 	@property
...
# 			   self.pydc == len(self.encrypted_data) + 1 and \
# 			   self.content_type != ContentType.INVALID and \
# 			   self.app_id != AppID.INVALID
...
#
# 	def _encrypt(self, key = None):  # пока заглушка: просто переводим текст в верхний регистр
# 		if self.app_id == AppID.MESSENGER and not self.content_type == ContentType.EMERGENCY: # если шифрование требуется
# 			nonce = urandom(8)
# 			self.encrypted_data = nonce + pure_salsa20.salsa20_xor(key = key, nonce = nonce, message = self.raw_data)
# 				#list(self.raw_data.swapcase()) # типа шифрование
# 		else:
# 			self.encrypted_data = self.raw_data # bytes(...) ?
# 		self.pydc = len(self.encrypted_data) + 1
#
# 	def _decrypt(self, key = None):
# 		if self.src_addr == self.dev_addr and self.app_id == AppID.MESSENGER: # если дешифрование требуется и это нам
# 			self.raw_data = pure_salsa20.salsa20_xor(key = key, nonce = self.encrypted_data[:8], message = self.encrypted_data[8:])
# 		#bytes(self.encrypted_data).swapcase() # типа дешифрование
# 		else:
# 			self.raw_data = bytes(self.encrypted_data)
#
# 	def update(self):
# 		self._packet = bytearray(pack(f"<2I4B", self.src_addr, self.target_addr, (self.fragm_c << 4) + self.fragment,
# 									  self.app_id.value, self.pydc, self.content_type.value))
# 		self._packet.extend(self.encrypted_data)
#
...
# 		self.src_addr, self.target_addr = self.dev_addr if from_me else self.target_addr, self.src_addr
#
